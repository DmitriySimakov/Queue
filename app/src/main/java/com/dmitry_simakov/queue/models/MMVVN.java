...
public class MMVVN extends Model_V {
    ...
    public MMVVN() {
        name = "M/M/v/K/N, K=v";
        description = "Очередь M/M/v/K/N представляет собой модель, в которой заявки обслуживаются " +
                "без ожидания, количество поступающих заявок не превышает N, а максимальное " +
                "число заявок в системе ограничено числом K, есть v каналов и имеется " +
                "экспоненциальное время обслуживания.\n" +
                "Практическим применением модели M/M/v/K/N является расчет приборов " +
                "разговорного тракта, на которые поступает поток вызовов от ограниченного числа " +
                "источников (100 и менее). В частности, эта модель используется при расчете " +
                "блоков АВ ступени АИ АТСК и при расчете учрежденческих АТС малой емкости.";
