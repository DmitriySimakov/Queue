...
public class MMV extends Model_V {
    ...
    public MMV() {
        name = "M/M/v";
        description = "Очередь M/M/v представляет собой модель, в которой поступающие заявки " +
                "формируют единую очередь и подчиняются распределению Пуассона, есть v каналов и " +
                "имеется экспоненциальное время обслуживания.\n" +
                "Практическим применением модели M/M/v является расчет систем коммутации сообщений " +
                "и управляющих устройств систем куммутации каналов. В некоторых системах машинных " +
                "АТС по системе с ожиданием работают устройства разговорного тракта. По системе " +
                "с ожиданием работают междугородные каналы при ручном обслужувании.";
